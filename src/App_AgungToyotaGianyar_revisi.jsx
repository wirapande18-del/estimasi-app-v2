...
function PDFView({refx,form,totals}){
  ...
  return <div ref={refx} style={styles.pdfNew}>
    <div style={styles.pdfHeader}>
      <div>
        <h1 style={styles.pdfBranch}>Agung Toyota Gianyar</h1>
        <p style={styles.pdfInfo}>📍 Jl. Dharma Giri, Buruan, Kec. Blahbatuh, Kabupaten Gianyar, Bali 80852.</p>
        <p style={styles.pdfInfo}>☎️ Telepon (0361) 948888</p>
      </div>
      <div style={styles.pdfLogo}><span style={{color:"#174a8b"}}>Agung</span><span style={{color:"#d71920"}}>TOYOTA</span></div>
    </div>

    <div style={styles.pdfDoubleLine}></div>
    <h1 style={styles.pdfTitle}>ESTIMASI BIAYA</h1>

    <div style={styles.pdfDataGrid}>
      <div>
        <PdfInfo label="Nama Pelanggan" value={form.customer}/>
        <PdfInfo label="Contact Person" value={form.contact}/>
        <PdfInfo label="No. Telepon" value={form.phone}/>
        <PdfInfo label="Alamat" value={form.alamat}/>
      </div>
      <div>
        <PdfInfo label="No. Polisi" value={form.polisi}/>
        <PdfInfo label="Type Kendaraan" value={form.kendaraan}/>
        <PdfInfo label="No. Rangka / Thn" value={`${form.rangka || ""}${form.tahun ? " / " + form.tahun : ""}`}/>
        <PdfInfo label="Tanggal Estimasi" value={tanggal}/>
      </div>
    </div>

    <h3 style={styles.pdfSectionTitle}>BERBAYAR / CHARGEABLE</h3>
    <table style={styles.pdfTableNew}>
      <thead><tr><th>No.</th><th>Part No</th><th>Part Name</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead>
      <tbody>{partsChargeable.length ? partsChargeable.map((r,i)=><tr key={r.id||i}><td>{i+1}</td><td>{r.partNo}</td><td style={{textAlign:"left"}}>{r.partName}</td><td>{r.qty}</td><td>Rp {rupiah(r.price)}</td><td>Rp {rupiah(r.total)}</td></tr>) : <tr><td colSpan="6">-</td></tr>}</tbody>
    </table>

    {subletsChargeable.length > 0 && <>
      <table style={styles.pdfTableNew}>
        <thead><tr><th>No.</th><th>Sublet No</th><th>Sublet / Additional Job</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead>
        <tbody>{subletsChargeable.map((r,i)=><tr key={r.id||i}><td>{i+1}</td><td>{r.subletNo}</td><td style={{textAlign:"left"}}>{r.subletName}</td><td>{r.qty}</td><td>Rp {rupiah(r.price)}</td><td>Rp {rupiah(r.total)}</td></tr>)}</tbody>
      </table>
    </>}

    <table style={styles.pdfTableNew}>
      <thead><tr><th>No.</th><th>Jasa Pekerjaan</th><th>Rate</th><th>Price</th><th>Total</th></tr></thead>
      <tbody>{jasaChargeable.length ? jasaChargeable.map((r,i)=><tr key={r.id||i}><td>{i+1}</td><td style={{textAlign:"left"}}>{r.jasaName}</td><td>{r.rate}</td><td>Rp {rupiah(r.price)}</td><td>Rp {rupiah(r.total)}</td></tr>) : <tr><td colSpan="5">-</td></tr>}</tbody>
    </table>

    <div style={styles.pdfTotalRow}>
      <b>Total Parts&nbsp;&nbsp;:&nbsp;&nbsp; Rp {rupiah(totals.totalParts)}</b>
      <b>Total Jasa&nbsp;&nbsp;:&nbsp;&nbsp; Rp {rupiah(totalJasaGabung)}</b>
    </div>
    <div style={styles.pdfGrand}>Grand Total&nbsp;&nbsp;:&nbsp;&nbsp; Rp {rupiah(totals.grandTotal)}</div>
    <p style={styles.pdfDate}>Denpasar, {tanggal}</p>

    <div style={styles.pdfBottom}>
      <div style={styles.pdfNote}>
        <b>Keterangan:</b><br/>
        Harga diatas sudah termasuk PPN. Item Part, Pekerjaan dan Harga diatas tidak mengikat sewaktu-waktu dapat berubah. Setelah disetujui mohon ditanda tangani lalu di whatsapp message ke No. 08113984343.<br/><br/>
        Untuk memudahkan pendaftaran saat kedatangan servis, silahkan Booking ke Petugas Estimasi atau whatsapp message ke No. 08113984343.
      </div>
...
pdfNew:{width:820,background:"white",padding:38,margin:"30px auto",fontFamily:"Arial",color:"#111",fontSize:12},
pdfHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start"},
pdfBranch:{fontSize:30,margin:"0 0 14px",fontWeight:"800"},
pdfInfo:{margin:"9px 0",fontSize:13},
pdfLogo:{fontSize:31,fontWeight:"800",marginTop:10,letterSpacing:-1},
pdfDoubleLine:{borderTop:"4px double #111",margin:"18px 0 26px"},
pdfTitle:{textAlign:"center",fontSize:28,margin:"0 0 26px",fontWeight:"800"},
pdfDataGrid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:35,marginBottom:18},
pdfInfoRow:{display:"grid",gridTemplateColumns:"120px 12px 1fr",gap:3,margin:"8px 0",alignItems:"start"},
pdfSectionTitle:{textAlign:"center",fontSize:14,margin:"20px 0 8px",fontWeight:"800"},
pdfTableNew:{width:"100%",borderCollapse:"collapse",margin:"8px 0 12px",fontSize:12},
pdfTotalRow:{display:"flex",justifyContent:"space-between",borderTop:"2px solid #111",paddingTop:10,marginTop:12,fontSize:13},
