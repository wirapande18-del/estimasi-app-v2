...
const emptyForm = {
  ...
  staff: "", jabatan: "", kepalaBengkel: "I Nengah Andika",
...
export default function App() {
  ...
  const [history, setHistory] = useState(() => load("estimasi_history_v4", []));
  const [partDb, setPartDb] = useState(() => load("sparepart_db_v4", defaultParts));
  const [customerDb, setCustomerDb] = useState(() => load("customer_db_v4", []));
  const [repairType, setRepairType] = useState("");

  useEffect(() => save("estimasi_history_v4", history), [history]);
  useEffect(() => save("sparepart_db_v4", partDb), [partDb]);
  useEffect(() => save("customer_db_v4", customerDb), [customerDb]);
  ...
  const totals = useMemo(() => {
    ...
    const discParts = totalParts * ((Number(form.discPartPct) || 0) / 100) + (Number(form.discPartRp) || 0);
    const discJasa = totalJasa * ((Number(form.discJasaPct) || 0) / 100) + (Number(form.discJasaRp) || 0);
    return { totalParts, totalSublet, totalJasa, discParts, discJasa, grandTotal: totalParts + totalSublet + totalJasa - discParts - discJasa };
  }, [form]);
  ...
  return <div onPaste={handlePaste} style={styles.app}>
    ...
    <main style={styles.main}>{page==="estimasi" && <Estimasi form={form} setField={setField} addPart={addPart} updateRow={updateRow} deleteRow={deleteRow} addSublet={addSublet} addJasa={addJasa} totals={totals} repairType={repairType} setRepairType={setRepairType} partDb={partDb} findCustomerByPlate={findCustomerByPlate} saveMasterFromCurrent={saveMasterFromCurrent} saveHistory={saveHistory} downloadPDF={downloadPDF} resetForm={resetForm} filePartRef={filePartRef} addPartsFromImage={addPartsFromImage} pdfRef={pdfRef}/>} {page==="history" && <History history={history} editHistory={editHistory} setHistory={setHistory} newForm={() => {resetForm(); setPage("estimasi")}}/>} {page==="sparepart" && <MasterPart partDb={partDb} setPartDb={setPartDb}/>} {page==="customer" && <MasterCustomer customerDb={customerDb} setCustomerDb={setCustomerDb}/>}</main>
  </div>;
}
...
function Estimasi(p){ const f=p.form; return <><section style={styles.card}><h2 style={{textAlign:"center"}}>FORM ESTIMASI</h2><div style={styles.tabs}><b>Profile</b><b>Estimasi Biaya</b></div><h3>🚙 Informasi Kendaraan</h3><div style={styles.grid4}><Input label="No Polisi *" value={f.polisi} onChange={(v)=>{p.setField("polisi",v); p.findCustomerByPlate(v)}}/><Input label="Type Kendaraan *" value={f.kendaraan} onChange={(v)=>p.setField("kendaraan",v)}/><Input label="No Rangka *" value={f.rangka} onChange={(v)=>p.setField("rangka",v)}/><Input label="Tahun *" value={f.tahun} onChange={(v)=>p.setField("tahun",v)}/></div><h3>👤 Informasi Pelanggan</h3><div style={styles.grid2}><Input label="Nama Pelanggan *" value={f.customer} onChange={(v)=>p.setField("customer",v)}/><Input label="No Telepon" value={f.phone} onChange={(v)=>p.setField("phone",v)}/><Input label="Contact Person" value={f.contact} onChange={(v)=>p.setField("contact",v)}/><Input label="Alamat" value={f.alamat} onChange={(v)=>p.setField("alamat",v)}/></div><h3>▣ Informasi Pembuat Estimasi</h3><div style={styles.grid3}><Input label="Nama Staff *" value={f.staff} onChange={(v)=>p.setField("staff",v)}/><Input label="Jabatan" value={f.jabatan} onChange={(v)=>p.setField("jabatan",v)}/><Input label="Kepala Bengkel" value={f.kepalaBengkel} onChange={(v)=>p.setField("kepalaBengkel",v)}/></div></section>
  ...
  <h3>🔨 Estimasi Sublet / Additional Job</h3><EditableSimple rows={f.sublets} type="sublets" updateRow={p.updateRow} deleteRow={p.deleteRow} cols={["subletNo","subletName","qty","price"]}/><button onClick={p.addSublet} style={styles.btn}>⊕ Add Sublet</button><hr/><h3>🧰 Jasa Pekerjaan</h3><EditableSimple rows={f.jasa} type="jasa" updateRow={p.updateRow} deleteRow={p.deleteRow} cols={["jasaName","rate","price"]}/><button onClick={p.addJasa} style={styles.btn}>⊕ Add Jasa</button><div style={styles.discount}><Input label="Disc Part (%)" value={f.discPartPct} onChange={(v)=>p.setField("discPartPct",v)}/><Input label="Disc Part (Rp)" value={f.discPartRp} onChange={(v)=>p.setField("discPartRp",toNumber(v))}/><Input label="Disc Jasa (%)" value={f.discJasaPct} onChange={(v)=>p.setField("discJasaPct",v)}/><Input label="Disc Jasa (Rp)" value={f.discJasaRp} onChange={(v)=>p.setField("discJasaRp",toNumber(v))}/><div><b>Grand Total</b><h2>Rp {rupiah(p.totals.grandTotal)}</h2></div></div><div style={styles.actions}><button onClick={p.resetForm} style={styles.red}>↻ Reset</button><button onClick={p.saveMasterFromCurrent} style={styles.btn}>💾 Save Master</button><button onClick={p.saveHistory} style={styles.green}>Submit / Save History</button><button onClick={p.downloadPDF} style={styles.blue}>Submit & Create PDF</button></div></section><PDFView refx={p.pdfRef} form={f} totals={p.totals}/></> }
